...
Puedes instalar la versión de desarrollo de `peruocc` directamente desde
GitHub:
...
1.  **`unidad_sf`**: Objeto espacial (`sf`) con el polígono oficial
    validado y proyectado en WGS84 (EPSG:4326).
2.  **`ocurrencias`**: Dataframe estandarizado alineado con el estándar
    **Darwin Core**.
3.  **`resumen`**: Estadísticas de la consulta (total de registros,
    desglose por fuente, reinos y cobertura).
4.  **`parametros`**: Metadatos de la llamada (fechas, límites y
    opciones empleadas).
